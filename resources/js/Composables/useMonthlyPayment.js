...
export const useMonthlyPayment = (total, interestRate, duration) => {
    const monthlyPayment = computed(() => {
        ...
        const monnthlyInterest = (isRef(interestRate) ? interestRate.value : interestRate) / 100 / 12;
        const numberOfPaymentMonths = (isRef(duration) ? duration.value : duration) * 12;

        return (
            (principle *
                monnthlyInterest *
                Math.pow(1 + monnthlyInterest, numberOfPaymentMonths)) /
            (Math.pow(1 + monnthlyInterest, numberOfPaymentMonths) - 1)
        );
    });
